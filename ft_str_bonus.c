...
char	*ft_date_iso(t_list *my)
{
	...
	str = ft_itoa_base_unint(base, my, va_arg(my->ptr, int), 10);
	str = ft_str_join_char_free(str, my->len, '-');
	len = my->len + 1;
	buf = ft_itoa_base_unint(base, my, va_arg(my->ptr, int), 10);
...
char	*ft_notprintable(char *str, t_list *my)
{
	...
	while (str[++i])
		if ((str[i] >= 0 && str[i] < 32) || str[i] == 127)
			++j;
	my->len = i + j;
	return (ft_notprintable_str(str, my->len, i, j));
}
